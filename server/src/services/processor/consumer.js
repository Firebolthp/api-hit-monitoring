 ...
 * EventConsumer
 *
 * Consumes API monitoring events from RabbitMQ and orchestrates:
 * - Message validation
 ...
 * - Event processing
 *
 * This class acts as the entry point of the Processor Service.
 */
...
class EventConsumer {
    /**
 * Creates a consumer instance and injects all required dependencies.
 *
...
/**
 * Starts the consumer lifecycle.
 *
 * Steps:
 * 1. Connect databases.
 * 2. Connect RabbitMQ.
 * 3. Configure prefetch.
 * 4. Register channel listeners.
...
/**
 * Establishes MongoDB and PostgreSQL connections.
 *
 * Retries multiple times before failing startup.
 */
...
/**
 * Attempts RabbitMQ reconnection after unexpected channel closure.
 *
 * Uses delayed retry to avoid aggressive reconnect storms.
 */
async _reconnect() {
        try {
...
/**
 * Parses and validates incoming RabbitMQ messages.
 *
 * Validation ensures malformed messages never reach
...
/**
 * Routes messages to the appropriate processor
 * based on event type.
 */
 ...
 *
 * Messages placed in DLQ require manual investigation
...
/**
 * Re-publishes a failed message back to RabbitMQ
 * after a delay determined by RetryStrategy.
 *
 * Uses exponential backoff to reduce pressure
 * on failing downstream services.
 */
...
/**
 * Runtime retry policy used during message processing.
 *
 * Example:
 * Attempt 1 -> 5s
 * Attempt 2 -> 10s
...
/**
 * Handles consumer startup retries.
 *
 * Prevents immediate process termination when
 * RabbitMQ or databases are temporarily unavailable.
 */
